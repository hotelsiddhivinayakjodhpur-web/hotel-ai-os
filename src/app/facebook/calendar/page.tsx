...
export default async function FacebookCalendarPage() {
  // Reuses the Content AI calendar, filtered to the Facebook channel.
  const all = await getContentCalendar(45);
  const days = all
    .map((d) => ({ ...d, items: d.items.filter((i) => i.channel === "FACEBOOK") }))
    .filter((d) => d.items.length > 0);

  return (
    <div>
      <PageHeader title="Posting Calendar" subtitle="Scheduled Facebook content for the next 45 days — publishing is manual" />
      <FacebookNav />

      {days.length === 0 ? (
        <EmptyState
          title="No Facebook posts scheduled"
      ...
      <p className="mt-6 text-[11px] text-muted">
        Manage schedules from the <Link href="/facebook/queue" className="underline hover:text-text">Content Queue</Link>.
      </p>
    </div>
  );
}
