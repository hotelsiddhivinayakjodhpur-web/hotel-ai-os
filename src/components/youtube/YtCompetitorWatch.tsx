...
import { addYtCompetitorAction } from "@/app/youtube/actions";
import type { CompetitorView } from "@/server/services/instagram.service";
import { Pill } from "@/components/ui/primitives";

/**
 * YouTube Competitor Watch — MANUAL mode, platform-scoped UI over the shared
 * CompetitorNote table + service (no duplicate storage or engine). The operator
 * records observed subscriber counts/notes; no scraping, no automation.
 */
const inputCls =
...
export function YtCompetitorWatch({ competitors }: { competitors: CompetitorView[] }) {
  ...
  function save() {
    start(async () => {
      const res = await addYtCompetitorAction({
        handle,
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <div className="card space-y-3">
        ...
        <p className="text-[11px] text-muted">Open the competitor&apos;s channel on YouTube and note what you see — only real observations are stored.</p>
        <input value={handle} onChange={(e) => setHandle(e.target.value)} placeholder="Competitor channel name" className={inputCls} />
        <input value={followers} onChange={(e) => setFollowers(e.target.value.replace(/[^\d]/g, ""))} placeholder="Subscribers seen (optional)" className={inputCls} />
        <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Note (e.g. weekly shorts, room tours perform well)" className={inputCls} />
      ...
      <div className="card">
        <h3 className="mb-3 text-sm font-semibold text-text">Tracked channels</h3>
                ...
                <li key={c.handle} className="rounded-lg border border-border bg-bg/40 p-2.5 text-sm">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="flex items-center gap-2">
                      {c.latestFollowers != null && <Pill tone="muted">{c.latestFollowers.toLocaleString()} subs</Pill>}
